...
const Certificates = () => (
  <div className="pb-20">
    <div className="bg-gradient-to-br from-purple-600 to-pink-600 px-5 pt-10 pb-8"><h1 className="text-white font-black text-2xl mb-1">Certificates</h1><p className="text-purple-100 text-sm">Your achievements & awards</p></div>
    <div className="px-5 pt-5 space-y-4">
      {CERTIFICATES.map((c,i)=>(
        <div key={i} className={`bg-gradient-to-br ${c.color} rounded-2xl p-5 text-white`}>
          <div className="flex items-start justify-between"><div><div className="text-xs font-semibold text-white/70 uppercase tracking-wide mb-1">ReLoop Certificate</div><div className="font-black text-2xl mb-1">{c.title}</div><div className="text-white/80 text-sm">{c.subtitle}</div><div className="text-xs text-white/60 mt-1">Vidya Mandir Estancia · Tamil Nadu</div></div><span className="text-5xl">{c.icon}</span></div>
          <button className="mt-4 bg-white/20 backdrop-blur border border-white/30 text-white text-sm font-bold px-4 py-2 rounded-xl flex items-center gap-2 active:scale-95 transition-transform"><Ic n="download" s={16}/> Download</button>
        </div>
      ))}
    </div>
  </div>
